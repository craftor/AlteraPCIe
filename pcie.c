...
static ssize_t sg_read(struct file *file, char __user *buf, size_t count, loff_t *pos)
{
	...
	copy_to_user(buf, (void *)ape->buffer_virt, PAGE_SIZE*ape->page_num);
...
static ssize_t sg_write(struct file *file, const char __user *buf, size_t count, loff_t *pos)
{
	...
	copy_from_user((void *)ape->buffer_virt, buf, PAGE_SIZE*ape->page_num);
...
static void sg_exit(struct ape_dev *ape)
{
	...
	// free the DMA buffer
	if (ape->buffer_virt) {
		pci_free_consistent(ape->pci_dev, PAGE_SIZE*ape->page_num, ape->buffer_virt, ape->buffer_bus);
	}
