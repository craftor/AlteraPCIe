...
struct ape_dev {
	...
	u8 * buffer_virt;
	// number of PAGE_SIZE to be allocated
	int page_num;   
	int step;         
...
static int __devinit dma_test(struct ape_dev *ape, struct pci_dev *dev)
{
	...
	printk(KERN_DEBUG "Allocated cache-coherent DMA buffer \n (virtual address = 0x%016llx, bus address = 0x%016llx).\n",
...
static int __devinit probe(struct pci_dev *dev, const struct pci_device_id *id)
{
	...
	ape->pci_dev = dev;
	dev->dev.platform_data = (void *)ape;
	...
	if (!pci_set_dma_mask(dev, DMA_BIT_MASK(32))) {
		printk(KERN_DEBUG "Could not set DMA mask.\n");
...
static void __devexit remove(struct pci_dev *dev)
{
	...
	if ((dev == 0) || (dev->dev.platform_data == 0) ) {
		printk(KERN_DEBUG "remove(dev = 0x%p) dev->dev.platform_data = 0x%p\n", dev, dev->dev.platform_data);
		//printk(KERN_DEBUG "remove(dev = 0x%p)", dev);
		return;
	}
	ape = (struct ape_dev *)dev->dev.platform_data;
	printk(KERN_DEBUG "remove(dev = 0x%p) where dev->dev.platform_data = 0x%p\n", dev, ape);
	//printk(KERN_DEBUG "remove(dev = 0x%p", dev);
	if (ape->pci_dev != dev) {
...
static int sg_open(struct inode *inode, struct file *file)
{
	...
	//sg_alloc_table(ape->sgm, MAX_CHDMA_SIZE, GFP_KERNEL);
...
{
	struct ape_dev *ape = (struct ape_dev *)file->private_data;
	struct ape_chdma_header *read_header = (struct ape_chdma_header *)ape->bar[APE_BAR_HEADER] + 1;
	int i, n = 0, irq_count;	
	...
	if (!read_header || !ape->table_virt || !ape->buffer_virt){
		printk(KERN_DEBUG "Could not allocate coherent DMA buffer.\n");
		goto fail;
	}
	printk(KERN_DEBUG "DMA buffer (virtual addr = 0x%016llx, bus addr = 0x%016llx).\n",
...
static ssize_t sg_write(struct file *file, const char __user *buf, size_t count, loff_t *pos)
{
	...
	if (!write_header || !ape->table_virt || !ape->buffer_virt) {
		printk(KERN_DEBUG "Could not allocate coherent DMA buffer.\n");
		goto fail;
	}
	printk(KERN_DEBUG "DMA buffer (virtual addr = 0x%016llx, bus addr = 0x%016llx).\n",
	...
	iowrite32(w, &write_header->w0);
	printk(KERN_DEBUG "writing 0x%08x to 0x%p\n", (u32)((ape->table_bus >> 16) >> 16), (void *)&write_header->bdt_addr_h);
	iowrite32(pci_dma_h(ape->table_bus), &write_header->bdt_addr_h);
	printk(KERN_DEBUG "writing 0x%08x to 0x%p\n", (u32)(ape->table_bus & 0xffffffffUL), (void *)&write_header->bdt_addr_l);
	iowrite32(pci_dma_l(ape->table_bus), &write_header->bdt_addr_l);
...
fail:
	printk(KERN_DEBUG "DMA Write Failed...\n");
	return 0;
end:
	return result;
}
...
<<<<<<< HEAD
		...
		for (i=0; i < (ape->page_num/4); i++) {
			ape_chdma_desc_set(&ape->table_virt->desc[i], ape->buffer_bus + i*PAGE_SIZE*4, (u32)(ape->bar[0]+i*PAGE_SIZE*4), PAGE_SIZE*4);
...
static void sg_exit(struct ape_dev *ape)
{
	...
	// free the DMA buffer
	if (ape->buffer_virt) {
		pci_free_consistent(ape->pci_dev, ape->page_num * PAGE_SIZE, ape->buffer_virt, ape->buffer_bus);
	}
